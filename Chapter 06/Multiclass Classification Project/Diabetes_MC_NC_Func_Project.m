...
% Test to train multi class classification model for diabetes with mixed dataset - number and categorical data
% Name: Diabetes_MC_NC_Func_Project.m
% Input: Dataset of Diabetes_M.xlsx

dsPath = 'C:\\Artificial Intelligence Book\\Students\\Datasets\\Diabetes Dataset2\\';
sData = 'Dataset of Diabetes_MM.xlsx';
...
T.Gender = categorical(T.Gender);      % convert the Gender column to category column  
X = table(T.ID(M, :), T.No_Pation(M, :), T.Gender(M, :), T.AGE(M,:), T.Urea(M,:), T.Cr(M, :), T.HbA1c(M, :) + ...
                  T.Chol(M, :), T.TG(M, :), T.HDL(M,:), T.LDL(M,:), T.VLDL(M, :), T.BMI(M, :));
Y = T.CLASS(M, :);                                  % get response variable
TLabels = T.CLASS(N, :);                        % get testing data

% Get testing data XX to perform prediction or classification
XX = table(T.ID(N, :), T.No_Pation(N, :), T.Gender(N, :), T.AGE(N,:), T.Urea(N,:), T.Cr(N, :), T.HbA1c(N, :) + ...
...
mdl_2 = fitcecoc(X, Y, "ClassNames", ["N", "P", "Y"]);
